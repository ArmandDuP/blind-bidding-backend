...
module.exports = function (io) {
  ...
  function startNextItemBidding(roomCode) {
    ...
    if (room.currentItemIndex >= room.currentItems.length) {
      biddingNamespace.to(roomCode).emit("biddingComplete");
      console.log(
    ...
    biddingNamespace.to(roomCode).emit("newItemToBid", { item: currentItem });
    console.log(
      `Room ${roomCode}: Bidding started for item ${currentItem.name}`
    );
  }
  ...
  function resolveCurrentItem(roomCode) {
    ...
    biddingNamespace.to(roomCode).emit("itemBidResult", {
      result,
  ...
  function resolveDrinks(roomCode) {
    ...
    for (const [giverId, { targetId, drinkId }] of Object.entries(
      room.attacks
    )) {
    ...
    if (survivors.length === 1) {
      biddingNamespace
        .to(roomCode)
        .emit("gameOver", { winner: survivors[0].name });
    ...
    room.attacks = {};
    biddingNamespace.to(roomCode).emit("attackResults", {
      results,
      players: survivors,
    });
  }
  ...
  biddingNamespace.on("connection", (socket) => {
    ...
    socket.on("attack", ({ roomCode, targetId, weaponId }, callback) => {
      ...
      if (
        Object.keys(room.attacks).length === Object.keys(room.players).length
      ) {
